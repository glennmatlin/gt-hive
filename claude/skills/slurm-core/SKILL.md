---
name: slurm-core
description: Use when the user needs portable Slurm guidance — batch scripts (sbatch/srun/salloc), job arrays, dependencies, sacct/squeue monitoring, and debugging pending or failed jobs. Pair with a site overlay (pace-phoenix or pace-ice) when a specific cluster is named; not for a named cluster alone.
---
...
- Recommend the user review every shell command before running it on a shared cluster. This is the interactive-session default. In an authorized unattended run the run-authority ledger controls instead (the harness's `[RUN AUTHORITY]` contract; see **Remote Cluster Deployment** above): prepare the full dependency chain up front and take one approval at the deployment boundary rather than pausing between stages.
