---
name: pace-phoenix
description: Use when working on Georgia Tech PACE Phoenix — "Phoenix", pace-quota, inferno, embers, A100/H100/H200/L40S GPUs, and Phoenix accounts, partitions, or QOS. Always pair with slurm-core for portable Slurm; not for ICE (that's pace-ice).
---
...
4. **Keep risky operations in an operator-controlled terminal.** Job
   ...
   delegated to background tooling. This is the interactive-session
   default. In an authorized unattended run, the run-authority ledger
   governs instead: the operator approves the deployment and its
   dependency chain once, up front, and that single approval — not a
   per-stage prompt — authorizes the chain (see `slurm-core`'s Remote
   Cluster Deployment).
