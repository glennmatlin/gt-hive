# AGENTS.md — gt-hive Codex wrapper

This wrapper is the always-on context layer for Codex (OpenAI) in the gt-hive
...
## Codex permission rules (dotfiles pattern)

For repeatable approval-policy management, keep Codex prefix rules in
dotfiles and symlink them into `~/.codex`:

```bash
mkdir -p ~/dotfiles/codex/rules
cp ~/.codex/rules/default.rules ~/dotfiles/codex/rules/default.rules
ln -sfn ~/dotfiles/codex/rules/default.rules ~/.codex/rules/default.rules
```

Add or edit rules in `~/dotfiles/codex/rules/default.rules` using
`prefix_rule(...)` entries:
...
prefix_rule(pattern=["ruff", "format"], decision="allow")
```

After changing rules, restart Codex to ensure the active session reads the
updated policy file.
